# TP CICD

## GUERRA Daniel, SANNA Thomas

### Livrables

- l'audit du système : dans rapport.pdf page 1
- les solutions proposées : dans rapport.pdf page 2
- la description du pipeline CI/CD : dans rapport.pdf page 2-3
- un schéma d’architecture : dans rapport.pdf page 3
- un dépôt Git contenant le projet : dans GitHub ^^
- le pipeline CI/CD : expliquée dans rapport.pdf page 3-4, et visible dans le dépôt GitHub
- les scripts d’automatisation : visibles dans le dépôt GitHub